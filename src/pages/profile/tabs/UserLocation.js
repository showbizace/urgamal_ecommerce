...
const UserLocation = () => {
	...
	const getShippingData = async () => {
		...
		await fetch(`http://3.0.91.173:8080/user/address`, requestOptions)
			.then((req) => req.json())
			.then((res) => {
				if (res.success === true) {
					setLoading(false);
	...
	const SubmitCreateShippingData = async (values) => {
		...
		await fetch(`http://3.0.91.173:8080/user/address`, requestOption)
	...
	return (
		<div style={{ width: "72.5%" }} className="flex flex-col">
			...
			<ProductModal
				...
				close={close}
				onSubmit={
					editingProdData?.create ? SubmitCreateShippingData : SubmitUpdateShippingData
				}
			/>
		</div>
	);
